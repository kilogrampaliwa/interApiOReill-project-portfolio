...
# Testing position control endingpoint
def test_read_main():
    response = client.get("/")
    assert response.status_code == 200
    assert response.json() == {"message": "API test succeeded"}

# Testing endingpoint /v0/players/
def test_read_players():
    response = client.get("/v0/players/?skip=0&1limit=10000")
    assert response.status_code == 200
...
# Testing endpoint /vo/performances/
